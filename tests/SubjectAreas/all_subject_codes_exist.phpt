...
exec('grep -e "<subject>.*</subject>" -o '.$data_dir.'/all-courses.xml  | grep -e "[A-Z]{3,4}" -o | sort | uniq > '.__DIR__.'/newsubs');
exec('grep -o -e "^[A-Z]+" '.$data_dir.'/subject_codes.csv > '.__DIR__.'/oldsubs');
